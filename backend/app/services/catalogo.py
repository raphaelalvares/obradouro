...
from app.schemas.catalogo import PromoverServicoIn, ServicoCreate, ServicoUpdate
...
async def excluir(session: AsyncSession, user_id: str, servico_id: uuid.UUID) -> None:
    ...
    try:
        ...
        # FK de template (Fatia 2) com ON DELETE RESTRICT → serviço em uso
        raise HTTPException(
            status.HTTP_409_CONFLICT,
            "serviço usado em um template — arquive-o em vez de excluir",
        ) from e
    except DBAPIError as e:
        raise (_map_42501(e) or e) from e
...
async def promover(session: AsyncSession, user_id: str, data: PromoverServicoIn) -> dict:
    ...
    if not nn:
        raise HTTPException(status.HTTP_422_UNPROCESSABLE_ENTITY, "descrição inválida")
    cmo = _custo_unit(data.valor_mo, data.quantidade)
    cmat = _custo_unit(data.valor_material, data.quantidade)
    ceq = _custo_unit(data.valor_equipamento, data.quantidade)
    ...
    criado = ja is None
    try:
        # tabela SEM trigger de seq → ON CONFLICT é seguro (não queima sequencial).
        row = (
            await session.execute(
                text(
                    """
