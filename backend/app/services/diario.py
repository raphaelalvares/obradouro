...
_SELECT = """
    select d.id, d.data, d.texto, d.clima, d.efetivo, d.seq_humano, d.created_by,
...
"""
_PATCH_COLS = ("data", "texto", "clima", "efetivo")
            ...
            await session.execute(
                text(
                    ...
                    insert into public.diario_obra
                      (id, obra_id, tenant_id, data, texto, clima, efetivo, created_by)
                    values (cast(:id as uuid), cast(:o as uuid), cast(:t as uuid), :data, :texto,
                            :clima, :efetivo, (select auth.uid()))
                    """
                ),
                {
...
async def atualizar(
    ...
    try:
        await session.execute(
            text(
                f"update public.diario_obra set {sets} "
                "where id = cast(:i as uuid) and obra_id = cast(:o as uuid)"
            ),
            params,
        )
    ...
    await log_event(
        ...
        entity_type="diario", entity_id=diario_id, changed=fields,
